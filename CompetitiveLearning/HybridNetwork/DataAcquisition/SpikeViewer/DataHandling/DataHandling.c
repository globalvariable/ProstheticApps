...
int is_bluespike_recording_data(char *bluespike_recording_data_path)
{
	...
	FILE *fp;
	int line_cntr = 0;
	strcpy(path, bluespike_recording_data_path);
	strcat(path, "/meta");
	if ((fp = fopen(path, "r")) == NULL)  { printf ("ERROR: SpikeViewer: Couldn't read file: %s\n\n", path); return 0; }
...
int get_format_version_for_bluespike_recording(int *version, char *bluespike_recording_data_path)
{
	...
	int line_cntr = 0;
	FILE *fp;
	if (is_bluespike_recording_data(bluespike_recording_data_path))
	{
		strcpy(path, bluespike_recording_data_path);
		strcat(path, "/meta");		
		if ((fp = fopen(path, "r")) == NULL)  { printf ("ERROR: SpikeViewer: Couldn't read file: %s\n\n", path); return 0; }
