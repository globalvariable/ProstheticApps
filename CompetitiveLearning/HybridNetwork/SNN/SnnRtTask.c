			...
			for (i = task_num; i < num_of_all_neurons; i+=SNN_SIM_NUM_OF_DEDICATED_CPUS)  // simulate the neurons for which this thread is responsible
			{
				...
				if (spike_generated)
				{
					...
					if (nrn->layer_type == NEURON_LAYER_TYPE_OUTPUT)
					{
						...
						{
							print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIRtTask", "hybrid_net_rl_bmi_internal_network_handler", "! write_to_neural_net_2_mov_obj_hand_msg_buffer().");
							goto EXIT;
						}
					}
				}	
