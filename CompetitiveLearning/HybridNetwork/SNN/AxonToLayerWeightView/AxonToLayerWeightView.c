...
bool create_axon_to_layer_weight_view_gui(GtkWidget *tabs)
{
	...
	for (i = 0; i < NUM_OF_AXON_TO_LAYER_GRAPHS; i++)
	{
		...
		if(!update_texts_of_combos_when_add_remove(combos_select_neuron_arr[i], blue_spike_network))
...
static void combos_select_neuron_func(GtkWidget *changed_combo)
{
	...
	if(!update_texts_of_combos_when_change(combos_select_neuron_arr[i], blue_spike_network, changed_combo))
