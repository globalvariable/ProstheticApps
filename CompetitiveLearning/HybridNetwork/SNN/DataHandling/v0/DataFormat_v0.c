...
static int create_main_meta_file(char *main_directory_path)
{
	...
	fprintf(fp,"learning_rate\t%.15f\n", get_hybrid_net_rl_bmi_data()->reward_data.learning_rate);
	...
	fclose(fp);
	print_message(INFO_MSG ,"HybridNetRLBMI", "DataFormat_v0", "create_main_meta_file", "created Main Meta File successfully");
	return 1;
}
...
static int load_main_meta_file(char *path_chooser, Network *in_silico_network, Network *blue_spike_network)
{
	...
	if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // learning_rate_scaler
	strcpy(word, line);
	word[20] = 0;
	if (strcmp(word, "learning_rate_scaler") != 0) 
		{ fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "Not learning_rate_scaler line."); }
	...
	{ 
		fclose(fp); 
		print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", line); 
		return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "Not a .............blue_spike_network............ line."); 
	}
	...
	{ 
		fclose(fp); 
		print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", line); 
		return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "Not a .............in_silico_network............ line."); 
	}
	...
	for(i = 0; i < layer_count; i++)
	{
		...
		for(j = 0; j < neuron_group_count; j++)	
		{
			...
			if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // IzhikevichNeuron   /// Poisson Neuron.
			if (strcmp(line, "IzhikevichNeuron\n") == 0)
			{
				...
				if (!add_iz_neurons_to_layer(in_silico_network, neuron_count, i, a, b, c+v_resting, d, k, 1.0/E, v_resting, v_threshold+v_resting, v_peak+v_resting, inhibitory, E_excitatory+v_resting, E_inhibitory+v_resting, -1.0/decay_rate_excitatory, -1.0/decay_rate_inhibitory, randomize_params))
					return print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIConfig", "load_main_meta_file", "! add_iz_neurons_to_layer().");	
			}
			if (strcmp(line, "PoissonNeuron\n") == 0)
			{
				if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  //  in_silico_network->layers[i]->neuron_groups[j]->neurons[k].poisson_params->firing_rate);		
				if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
				firing_rate = atof(word);

				if (! add_poisson_neurons_to_layer(in_silico_network, neuron_count, i, inhibitory, firing_rate))	
					return print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIConfig", "load_main_meta_file", "! add_poisson_neurons_to_layer().");	
			}			
		...
		for(j = 0; j < neuron_group_count; j++)	
		{
			...
			for (k_iter = 0; k_iter < neuron_count; k_iter++)
			{
				...
				for (m = 0; m < num_of_axons; m++)
				{
					...
					if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // blue_spike_network->layers[%u]->neuron_groups[%u]->neurons[%u].axon_list->to[%u]->syn_list->synapses[%u].plastic\t%.15f\n
					if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
		...
		for(j = 0; j < neuron_group_count; j++)	
		{
			...
			for (k_iter = 0; k_iter < neuron_count; k_iter++)
			{
				...
				for (m = 0; m < num_of_synapses; m++)
				{
					...
					if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // in_silico_network->layers[%u]->neuron_groups[%u]->neurons[%u].syn_list->synapses[%u].plastic\t%u\n
					if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
					plastic = strtoull(word, &end_ptr, 10);	

					if (plastic)
					{
						if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // in_silico_network->layers[%u]->neuron_groups[%u]->neurons[%u].syn_list->synapses[%u].ps_stdp_pre_post->change_amount\t%.15f\n
						if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
						change_stdp_pre_post = atof(word);		
	
						if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // in_silico_network->layers[%u]->neuron_groups[%u]->neurons[%u].syn_list->synapses[%u].ps_stdp_pre_post->decay_rate\t%.15f\n
						if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
						decay_rate_stdp_pre_post = atof(word);	

						if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // in_silico_network->layers[%u]->neuron_groups[%u]->neurons[%u].syn_list->synapses[%u].ps_eligibility->decay_rate\t%.15f\n
						if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
						eligibility_decay_rate = atof(word);	

						if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // in_silico_network->layers[%u]->neuron_groups[%u]->neurons[%u].syn_list->synapses[%u].ps_eligibility->max_eligibility\t%.15f\n
						if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
						max_eligibility = atof(word);	

						if (fgets(line, sizeof line, fp ) == NULL)   { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "fgets() == NULL."); }  // in_silico_network->layers[%u]->neuron_groups[%u]->neurons[%u].syn_list->synapses[%u].ps_eligibility->depol_eligibility\t%.15f\n
						if(!get_word_in_line('\t', 1, word, line, TRUE)) { fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "!get_word_in_line."); }
						...
						if (! create_ps_pre_post_stdp_for_synapse(this_neuron , change_stdp_pre_post, change_stdp_pre_post, -1.0/decay_rate_stdp_pre_post, -1.0/decay_rate_stdp_pre_post, m))
 							{ fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "! create_ps_pre_post_stdp_for_synapse."); }
	...
	print_message(INFO_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "Successfully read main meta file.");
