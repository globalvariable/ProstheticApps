...
int is_mov_obj_handler_data(char *mov_obj_handler_data_path)
{
	char path[600];
	char line[200];
	FILE *fp;
	int line_cntr = 0;
	strcpy(path, mov_obj_handler_data_path);
	strcat(path, "/meta");
	if ((fp = fopen(path, "r")) == NULL)  { printf ("ERROR: MovObjHandler: Couldn't read file: %s\n\n", path); return 0; }
	...
	fclose(fp);   
	if (strcmp(line, "----------MovObjHandler - Main Meta File----------\n" ) == 0)
		return 1;
	else
		return 0;	
}

int get_format_version(int *version, char *mov_obj_handler_data_path)
{
	char path[600];
	char line[200];
	char word[10];
	int line_cntr = 0;
	FILE *fp;
	if (is_mov_obj_handler_data(mov_obj_handler_data_path))
	{
		strcpy(path, mov_obj_handler_data_path);
		strcat(path, "/meta");		
